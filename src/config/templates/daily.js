...
function printSaleAccSummary(statRes, startDate) {
  let res = "三、" + startDate.toString().slice(0,4) + "年累计销售：全公司累计销售" + statRes.sumAccCurtQty.toFixed(decimalDigits) + "吨，同比增长" 
    ...
    + (statRes.sumAccCurtAmount / 10000).toFixed(decimalDigits) + "万元，同比增长" + (((statRes.sumAccCurtAmount - statRes.sumAccLastAmount) / statRes.sumAccLastAmount) * 100).toFixed() + '%；其中化肥'
    + statRes.subAcc + '('
