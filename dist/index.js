...
var loadModels = require('./config/loadModels');
...
var SaleIssueEntry = loadModels(sequelize, 'SaleIssueEntry');
var PurInEntry = loadModels(sequelize, 'PurInEntry');
...
var queryInventory = sequelize.query(sqlCommand.Inventory, {
  type: sequelize.QueryTypes.SELECT,
...
function statFert(arrData) {
  ...
  arrData.forEach(function (item) {
    brandF.forEach(function (brand) {
      // 这里brand其实就是brandF里的每一个元素，对其修改肯定会改变。
  ...
  /////
  //添加明细统计
  brandF.forEach(function (val) {
    val.details = statDetails(val.data, function (item) {
      return item.FBrandFertilizer == val.name;
    }, function (item) {
      return item.FMaterialType3;
    });
  });
...
function statDetails(arrData, filter, group) {
  var res = accObj = [];
  arrData.filter(filter).group(group).forEach(function (value, index) {
    var o = {
      key: value.key,
    ...
    accObj = value.data.reduce(function (acc, val) {
      acc.sumQty += val.FBaseQty;
...
function sumByColumnName(arrData, cn) {
  var sum = arrData.reduce(function (acc, val) {
    acc += Number(val[cn]);
    return acc;
  }, 0);
  return sum;
...
function groupAndSumByColumn(arrData, group, colName) {
  ...
  arrData = arrData.reduce(function (acc, val) {
    acc.push({
      name: val.key,
...
function statInventory(arrData, options) {
  ...
  var arrDataOri = arrData.slice(); //复制数组

  ///////////////////统计化肥总概括
  arrData = arrData.group(function (item) {
  ...
  arrData = arrData.filter(function (item) {
    return item.FBrandFertilizer == '尿素';
  }).group(function (item) {
    return item.CorrStorageOrgUnit;
  });
  arrData.map(function (item) {
  ...
  arrData.filter(function (item) {
    ...
    return item.FMaterial;
  }) //这里以物料名称为分组，相同名称不同规格的物料会被合并！
  .map(function (item) {
    var o = {
      key: item.key,
      name: item.key,
  ...
  arrData = arrDataOri.slice();
  arrData = arrData.filter(function (item) {
    return item.FBrandFertilizer != '尿素' && item.FBrandFertilizer != '其他';
  }).group(function (item) {
    return item.FBrandFertilizer;
  }) // 这里已经按品牌化肥分好类，格式为[{key: FBrandFertilizer, data:[....]},{key: FBrandFertilizer, data:[....]}]
  .reduce(function (acc, val) {
    var arr = filterAndGroupAndSumByColumn(val.data, { //这里有问题！
  ...
  arrData = arrDataOri.slice();
  arrData = arrData.filter(function (item) {
    return item.FBrandFertilizer == '其他';
  }).group(function (item) {
    return item.FMaterialType2;
  }) // 这里已经按品牌化肥分好类，格式为[{key: FMaterialType2, data:[....]},{key: FMaterialType2, data:[....]}]
  .reduce(function (acc, val) {
    acc.push({
      name: val.key,
...
////
function printDetailsSummary(arrData) {
  //数组要用reduce 一定要把空的剔除！
  var sumDetailsReport = void 0;
  if (arrData) {
    sumDetailsReport = arrData.reduce(function (acc, val) {
...
function printSaleSummary1(statRes, startDate) {
  var sumUrea = statRes.statFertRes.filter(function (item) {
    return item.name == '尿素';
  })[0].sumQty.toFixed(2) || 0;
  var sumUreaDetails = statRes.statUreaRes.reduce(function (acc, val) {
    acc.push(val.name + '[' + val.model + ']' + val.sumQty.toFixed(2) + "吨,单价" + (val.sumAmount / val.sumQty).toFixed(0));
    return acc;
  }, []).join(',');
  var sumFert = statRes.statFertRes.filter(function (item) {
    return item.name != '尿素' && item.sumQty != 0;
  }).reduce(function (acc, val) {
    acc.push(val.name + val.sumQty.toFixed(2) + '吨');
    return acc;
  }, []).join(',');
  var strSaleSummary = "销售：化肥总售出" + statRes.sumCurtQty.toFixed(2) + "吨，" + (statRes.sumCurtAmount / 10000).toFixed(2) + "万元。其中尿素" + sumUrea + "吨\（" + sumUreaDetails + "），" + sumFert + "。" + startDate.split('-')[0] + "年累计销售" + statRes.sumAccCurtQty.toFixed(2) + "吨，同比增长" + (statRes.sumAccCurtQty / statRes.sumAccLastQty * 100).toFixed() + "%；累计销额" + (statRes.sumAccCurtAmount / 10000).toFixed(2) + "万元，同比增长" + (statRes.sumAccCurtAmount / statRes.sumAccLastAmount * 100).toFixed() + "%（以销售出库单统计）。";
...
function printSaleSummary(statRes, startDate) {
  ...
  var strSaleSummary = "销售：化肥总售出" + statRes.sumCurtQty.toFixed(2) + "吨，" + (statRes.sumCurtAmount / 10000).toFixed(2) + "万元。" + sumFert + "。" + startDate.split('-')[0] + "年累计销售" + statRes.sumAccCurtQty.toFixed(2) + "吨，同比增长" + (statRes.sumAccCurtQty / statRes.sumAccLastQty * 100).toFixed() + "%；累计销额" + (statRes.sumAccCurtAmount / 10000).toFixed(2) + "万元，同比增长" + (statRes.sumAccCurtAmount / statRes.sumAccLastAmount * 100).toFixed() + "%（以销售出库单统计）。";
  return strSaleSummary;
}

function printPurSummary(statRes, startDate) {
  var sumFert = statRes.statFertRes.filter(function (item) {
    return item.sumQty != 0;
  }) //item.name != '尿素' && 
  .reduce(function (acc, val) {
    acc.push(val.name + val.sumQty.toFixed(2) + '吨' + "(" + printDetailsSummary(val.details) + ")");
    return acc;
  }, []).join(',');
  var strSaleSummary = "购进：化肥总购入" + statRes.sumCurtQty.toFixed(2) + "吨，" + (statRes.sumCurtAmount / 10000).toFixed(2) + "万元。" + sumFert + "。" + startDate.split('-')[0] + "年累计购入" + statRes.sumAccCurtQty.toFixed(2) + "吨，同比增长" + (statRes.sumAccCurtQty / statRes.sumAccLastQty * 100).toFixed() + "%；累计销额" + (statRes.sumAccCurtAmount / 10000).toFixed(2) + "万元，同比增长" + (statRes.sumAccCurtAmount / statRes.sumAccLastAmount * 100).toFixed() + "%（以采购入库单统计）。";
  return strSaleSummary;
}

function printInvtSummary(statRes, endDate) {
  var fert = '化肥总库存' + statRes.sumFertQty.toFixed(1) + '吨（其中' + statRes.summaryFert.reduce(function (acc, val) {
    acc.push(val.key + val.sumQty.toFixed(1) + "吨");return acc;
  }, []).join(',') + ')， ' + '尿素' + statRes.sumUreaQty.toFixed(1) + '吨（其中' + statRes.summartUrea.reduce(function (acc, val) {
    acc.push(val.key + val.sumQty.toFixed(1) + "吨");return acc;
  }, []).join(',') + ')， ' + '其中' + statRes.detailUrea.reduce(function (acc, val) {
    acc.push(val.key + val.sumQty.toFixed(1) + "吨");return acc;
  }, []).join(',') + ')， ' + statRes.detailFert.reduce(function (acc, val) {
    if (val.data.length == 1) acc.push(val.key + val.data[0].sum.toFixed(1) + '吨');else {
      acc.push(val.key + sumByColumnName(val.data, 'sum').toFixed(1) + '吨' + '(' + val.data.reduce(function (a, v) {
        a.push(v.name + v.sum.toFixed(1));return a;
      }, []).join(',') + ')');
    }
    return acc;
  }, []).join(',') + '，' + '其他肥' + sumByColumnName(statRes.detailOtherFert, 'sum').toFixed(0) + '吨（' + statRes.detailOtherFert.reduce(function (acc, val) {
    acc.push(val.name + val.sum + '吨');return acc;
  }, []).join(',') + ')。 ';
  return fert;
}
...
Promise.join(query, queryCurtAcc, queryLastAcc, queryPurIn, queryPurInCurtAcc, queryPurInLastAcc, queryInventory, function (curtData, curtAccData, lastAccData, curtPurData, curtPurAccData, lastPurAccData, invtData) {
  var statSaleRes = {
    ...
    statDetails: statDetails(curtData, function (v) {
      return v.FBrandCarbaMind != '非尿素';
    }, function (item) {
      return item.FMaterialNumber;
    })
  };
  var statPurRes = {
